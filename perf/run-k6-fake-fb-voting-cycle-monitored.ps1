...
param(
    ...
    [string]$ArtifactPrefix = 'fake-fb-voting-cycle-monitor',
    [string]$K6ArtifactPrefix = 'fake-fb-voting-cycle',
...
$runnerScript = Join-Path $PSScriptRoot 'run-k6-fake-fb-voting-cycle.ps1'
...
try {
    ...
    if ($SkipReadyCheck) { $runnerParams.SkipReadyCheck = $true }
    if ($SkipQueueDrainCheck) { $runnerParams.SkipQueueDrainCheck = $true }
