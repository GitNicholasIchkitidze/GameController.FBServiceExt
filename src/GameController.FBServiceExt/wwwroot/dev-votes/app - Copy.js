...
const ka = {
  ...
  onMessage: (showId) => `ვოტინგი ჩართულია. მონიტორი აკვირდება showId='${showId}'.`,
...
function fmtInt(value) { return new Intl.NumberFormat().format(Number(value || 0)); }
...
async function refreshSnapshot() {
  ...
  try {
    ...
    setMessage(votingSnapshot.votingStarted
      ? ka.onMessage(votesSnapshot.showId || votingSnapshot.activeShowId || '-')
      : ka.offMessage);
  } catch (error) {
...
showInput.addEventListener('keydown', (event) => { if (event.key === 'Enter') { event.preventDefault(); currentPage = 1; refreshSnapshot(); } });
recentFilterInput.addEventListener('keydown', (event) => { if (event.key === 'Enter') { event.preventDefault(); currentPage = 1; refreshSnapshot(); } });
pageSizeSelect.addEventListener('change', () => { currentPage = 1; refreshSnapshot(); });
prevPageButton.addEventListener('click', () => { if (currentPage > 1) { currentPage--; refreshSnapshot(); } });
nextPageButton.addEventListener('click', () => { currentPage++; refreshSnapshot(); });
setDefaultText();
setDefaultRange();
applyTimer();
refreshSnapshot();
